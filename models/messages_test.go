...
var (
	messageDBTypes = map[string]string{`ID`: `integer`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`, `Sender`: `text`, `ChannelName`: `text`, `Message`: `text`}
	_              = bytes.MinRead
)
